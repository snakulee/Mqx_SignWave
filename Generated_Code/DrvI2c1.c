...
typedef DrvI2c1_TDeviceData *DrvI2c1_TDeviceDataPtr; /* Pointer to the device data structure. */

/* {MQXLite RTOS Adapter} Static object used for simulation of dynamic driver memory allocation */
static DrvI2c1_TDeviceData DeviceDataPrv__DEFAULT_RTOS_ALLOC;
...
void DrvI2c1_Interrupt(LDD_RTOS_TISRParameter _isrParameter)
{
  ...
  DrvI2c1_TDeviceDataPtr DeviceDataPrv = (DrvI2c1_TDeviceDataPtr)_isrParameter;
  ...
  if (I2C_PDD_GetMasterMode(I2C1_BASE_PTR) == I2C_PDD_MASTER_MODE) { /* Is device in master mode? */
    if (I2C_PDD_GetTransmitMode(I2C1_BASE_PTR) == I2C_PDD_TX_DIRECTION) { /* Is device in Tx mode? */
        ...
        if ((DeviceDataPrv->SerFlag & ADDR_COMPLETE) != 0x00U) { /* If 10-bit addr has been completed */
          if (DeviceDataPrv->OutLenM != 0x00U) { /* Is any char. for transmitting? */
            ...
            if (DeviceDataPrv->InpLenM != 0x00U) { /* Is any char. for reception? */
              ...
              DrvI2c1_OnMasterBlockSent(DeviceDataPrv->UserData); /* Invoke OnMasterBlockSent event */
            }
          }
        } else {
      ...
      *(DeviceDataPrv->InpPtrM)++ = I2C_PDD_ReadDataReg(I2C1_BASE_PTR); /* Receive character */
      if (DeviceDataPrv->InpLenM == 0x00U) { /* Is any char. for reception? */
        DrvI2c1_OnMasterBlockReceived(DeviceDataPrv->UserData); /* Invoke OnMasterBlockReceived event */
      }
    }
  } else {
  ...
  if (ErrorMask != 0x00U) {            /* Is any error mask set? */
    DrvI2c1_OnError(DeviceDataPrv->UserData); /* If yes then invoke user event */
  }
}
...
LDD_TDeviceData* DrvI2c1_Init(LDD_TUserData *UserDataPtr)
{
  ...
  DrvI2c1_TDeviceData *DeviceDataPrv;
  ...
  DeviceDataPrv->SavedISRSettings.isrFunction = _int_install_isr(LDD_ivIndex_INT_I2C1, DrvI2c1_Interrupt, DeviceDataPrv);
  DeviceDataPrv->SerFlag = ADDR_7;     /* Reset all flags start with 7-bit address mode */
  DeviceDataPrv->SlaveAddr = 0x90U;    /* Set variable for slave address */
  DeviceDataPrv->SendStop = LDD_I2C_SEND_STOP; /* Set variable for sending stop condition (for master mode) */
  ...
  /* I2C1_F: MULT=0,ICR=0x10 */
  I2C1_F = I2C_F_ICR(0x10);            /* Set prescaler bits */
  I2C_PDD_EnableDevice(I2C1_BASE_PTR, PDD_ENABLE); /* Enable device */
  I2C_PDD_EnableInterrupt(I2C1_BASE_PTR); /* Enable interrupt */
  /* Registration of the device structure */
  PE_LDD_RegisterDeviceStructure(PE_LDD_COMPONENT_DrvI2c1_ID,DeviceDataPrv);
...
void DrvI2c1_Deinit(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  (void)_int_install_isr(LDD_ivIndex_INT_I2C1, ((DrvI2c1_TDeviceDataPtr)DeviceDataPtr)->SavedISRSettings.isrFunction, ((DrvI2c1_TDeviceDataPtr)DeviceDataPtr)->SavedISRSettings.isrData);
  /* Unregistration of the device structure */
  PE_LDD_UnregisterDeviceStructure(PE_LDD_COMPONENT_DrvI2c1_ID);
...
{
  DrvI2c1_TDeviceData *DeviceDataPrv = (DrvI2c1_TDeviceData *)DeviceDataPtr;
